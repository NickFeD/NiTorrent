    ...
    ITorrentEngineGateway engineGateway) : ITorrentCommandService
{
    ...
    private async Task<TorrentCommandResult> ExecuteAsync(TorrentId id, bool deleteData, CommandType commandType, CancellationToken ct)
    {
        ...
        if (commandType == CommandType.Remove && engineReady)
        {
            ...
            {
                return TorrentCommandResult.Failed(id, ex.Message);
            }
        }
        ...
        if (!engineReady)
        {
            ...
            {
                CommandType.Start => "Start queued until engine becomes available.",
                CommandType.Pause => "Pause queued until engine becomes available.",
                CommandType.Remove => "Remove queued until engine becomes available.",
                _ => null
            });
        }
