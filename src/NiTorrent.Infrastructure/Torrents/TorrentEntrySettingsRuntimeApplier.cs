...
/// <summary>
/// Applies per-torrent settings to the active MonoTorrent manager when it is available.
...
public sealed class TorrentEntrySettingsRuntimeApplier : ITorrentEntrySettingsRuntimeApplier
{
    ...
    private static bool TrySetProperty(object target, IEnumerable<string> candidateNames, object value)
    {
        foreach (var candidateName in candidateNames)
        {
            ...
            {
                // Reflective access is best-effort because MonoTorrent does not expose a stable public adapter for these settings.
            }
        }
